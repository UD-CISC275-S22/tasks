...
export function tripleNumbers(numbers: number[]): number[] {
    let tripled: number[] = [];
    tripled = numbers.map((num: number): number => num * 3);
    return tripled;
}
...
export const removeDollars = (amounts: string[]): number[] => {
    const ints: number[] = amounts.map((num: string): number => {
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    const words: string[] = messages.map((str: string): string => {
        if (str.endsWith("!")) {
    ...
    const filtered: string[] = words.filter((str: string) => str !== "");
    return filtered;
};
...
export function countShortWords(words: string[]): number {
    const count: string[] = words.filter(
