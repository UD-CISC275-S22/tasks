...
export function stringsToIntegers(numbers: string[]): number[] {
    if (numbers.length === 0) {
        ...
        const multiArray = numbers.map(Number);
        const numArray = multiArray.map((num: number): number =>
...
export const removeDollars = (amounts: string[]): number[] => {
    const noDollas = amounts.map((amount: string): string =>
        amount[0] === "$" ? amount.slice(1, amount.length) : amount
    );

    return stringsToIntegers(noDollas);
};
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    ...
    const uppCase = noQuestions.map((exclame: string): string =>
        exclame[exclame.length - 1] === "!" ? exclame.toUpperCase() : exclame
    );
    return uppCase;
};
