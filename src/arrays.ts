...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    ...
    messages = messages.map((message: string): string =>
        message.endsWith("!") ? (message = message.toUpperCase()) : message
    );
    return messages;
};
...
export function makeMath(addends: number[]): string {
    ...
    const addendStrings = addends.map(
        (num: number): string => num.toString() + "+"
    );
    const addendString = addendStrings.reduce(
        (currentString: string, str: string) => currentString + str,
        ""
    );
    return sum + "=" + addendString.slice(0, -1);
}
