...
export function tripleNumbers(numbers: number[]): number[] {
    return numbers.map((numb: number): number => numb * 3);
}
...
export function stringsToIntegers(numbers: string[]): number[] {
    return numbers.map((numb: string): number =>
        isNaN(parseInt(numb)) ? 0 : parseInt(numb)
    );
}
...
export const removeDollars = (amounts: string[]): number[] => {
    return amounts.map((numb: string): number => {
        if (numb[0] === "$") {
            numb = numb.slice(1);
        }
        return isNaN(parseInt(numb)) ? 0 : parseInt(numb);
    });
};
...
export function makeMath(addends: number[]): string {
    ...
    const sum: number = addends.reduce(
        (currentTotal: number, numb: number) => currentTotal + numb,
        0
    );
...
export function injectPositive(values: number[]): number[] {
    ...
    const sum: number = values.reduce((CurrentTotal: number, numb: number) => {
        if (numb >= 0 && !foundNegative) {
            CurrentTotal += numb;
        } else if (numb < 0) {
            foundNegative = true;
        }
