...
export const removeDollars = (amounts: string[]): number[] => {
    return amounts.map((str) => {
        // Remove "$" symbol if it exists at the beginning of the string
        const cleanedStr = str.replace(/^\$/, "");

        // Parse the cleaned string as an integer
        const parsedInt = parseInt(cleanedStr, 10);

        // Check if the parsing was successful, and return the result accordingly
        return isNaN(parsedInt) ? 0 : parsedInt;
    });
};
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    return messages
        .filter((message) => !message.endsWith("?")) // Remove strings ending in "?"
        .map((message) =>
