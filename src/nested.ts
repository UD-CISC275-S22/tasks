...
export function findQuestion(
    ...
    const specq: Question[] = questions.filter(
        (question: Question): boolean => question.id === id
    );
    return specq.length === 0 ? null : specq[0];
}
...
export function toCSV(questions: Question[]): string {
    const thing = "id,name,options,points,published\n";
...
export function editOption(
    ...
    return questions.map((question) => {
        if (question.id === targetId) {
            const beg: string[] = question.options.slice(0, targetOptionIndex);
            const end: string[] = question.options.slice(targetOptionIndex + 1);
            return {
                ...question,
                options:
                    targetOptionIndex === -1
