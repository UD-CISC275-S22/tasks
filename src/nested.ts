...
export function getPublishedQuestions(questions: Question[]): Question[] {
    const copy: Question[] = questions.filter(
...
export function sumPublishedPoints(questions: Question[]): number {
    return [...questions].reduce(
        (current: number, question: Question): number =>
            question.published ? (current += question.points) : current,
        0
    );
}
...
export function sameType(questions: Question[]): boolean {
    //let count: number = 0;
    /*const counter = questions.reduce(
        (current: Question, question: Question): number =>
            current.type === question.type ? (count += 1) : count,
        0
    );*/
