...
export function findQuestion(
    ...
    const ind = questions.find((question: Question) => question.id === id);
    return ind ? ind : null;
}
...
export function sumPublishedPoints(questions: Question[]): number {
    const publishedPoints = getPublishedQuestions(questions).reduce(
        (sum: number, question: Question) => sum + question.points,
        0
    );
...
export function sameType(questions: Question[]): boolean {
    const isSameType = questions.filter(
        (question: Question): boolean =>
            question.type === "short_answer_question"
    );
    return isSameType.length === questions.length || isSameType.length === 0;
}
...
export function renameQuestionById(
    ...
    const targetQuestion = questions.map(
...
export function duplicateQuestionInArray(
    ...
    const dupe = duplicateQuestion(newId, questions[ind]);
    newQuestions.splice(ind + 1, 0, dupe);
    return newQuestions;
}
