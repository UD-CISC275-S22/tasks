...
export function sumPublishedPoints(questions: Question[]): number {
    const sumAndPublishedPoints = sumPoints(getPublishedQuestions(questions));
    return sumAndPublishedPoints;
}
...
export function toCSV(questions: Question[]): string {
    ...
    const rows = questions.map((question: Question): string =>
        [
    ...
    return questions.map((question: Question): Question => {
        if (question.id !== targetId) {
            ...
            if (targetOptionIndex === -1) {
                ...
                const middle = { ...question, options: optionsArray };
                return middle;
            }
        }
    });
...
export function duplicateQuestionInArray(
    ...
    const dupIndex = questions.findIndex((question: Question): boolean => {
        return question.id === targetId;
    });
    const duppedQuestions = [...questions];
    duppedQuestions.splice(
        dupIndex + 1,
        0,
        duplicateQuestion(newId, questions[dupIndex])
    );
    return duppedQuestions;
}
