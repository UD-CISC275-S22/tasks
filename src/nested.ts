...
export function getNonEmptyQuestions(questions: Question[]): Question[] {
    return questions.filter(
        (questions) => questions.body != "" || questions.expected != ""
    );
}
...
export function findQuestion(
    ...
    return questions.find((q: Question | null) => q.id === id) || null;
}
...
export function sumPoints(questions: Question[]): number {
    return questions.reduce((s, qn) => s + qn.points, 0);
}
...
export function sumPublishedPoints(questions: Question[]): number {
    const pq = questions.filter((q) => q.published);
    return pq.reduce((s, qn) => s + qn.points, 0);
}
...
export function sameType(questions: Question[]): boolean {
    ...
    const f1 = questions[0].type;
    return questions.every((q) => q.type === f1);
}
...
export function editOption(
    ...
    return questions.map((q) => {
        if (q.id !== targetId) return q;
        const newq = [...q.options];
        if (targetOptionIndex === -1) {
            newq.push(newOption);
        } else {
            newq[targetOptionIndex] = newOption;
        }
        return { ...q, options: newq };
    });
}
...
export function duplicateQuestionInArray(
    ...
    const newQuestion = duplicateQuestion(newId, originalQuestion);
