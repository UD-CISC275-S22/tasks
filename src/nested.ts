...
export function getPublishedQuestions(questions: Question[]): Question[] {
    ...
    const publish = published.filter(
...
export function getNonEmptyQuestions(questions: Question[]): Question[] {
    const filt = (question: Question): boolean =>
        question.body !== "" ||
        (question.expected !== "" && question.options !== []);
    const publish = questions.filter(filt);
    return publish;
}
...
export function findQuestion(
    ...
    const found = questions.find((element) => element.id === id);
    if (found === undefined) {
        return null;
    } else {
        return found;
...
export function removeQuestion(questions: Question[], id: number): Question[] {
    const filt = (question: Question): boolean => question.id !== id;
    const removed = questions.filter(filt);
    return removed;
}
...
export function sameType(questions: Question[]): boolean {
    ...
    const value = questions[1].type;
    const same = questions.every(
        (question: Question): boolean => question.type === value
    );
    return same;
}
...
export function changeQuestionTypeById(
    ...
    changed[finding].type = newQuestionType;
    if (newQuestionType !== "multiple_choice_question") {
        changed[finding].options = [];
    }
    return changed;
}
...
export function duplicateQuestionInArray(
    ...
    const duplicate = duplicateQuestion(newId, changed[finding]);
    changed.splice(finding + 1, 0, duplicate);
    return changed;
}
