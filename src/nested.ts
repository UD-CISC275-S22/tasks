...
export function sumPublishedPoints(questions: Question[]): number {
    const publishedQuestions = questions.reduce(
        (currentPublished: number, question: Question) =>
            question.published
                ? (currentPublished += question.points)
                : (currentPublished += 0),
        0
    );
    return publishedQuestions;
...
export function toCSV(questions: Question[]): string {
    const questionsCSV = questions
        ...
        .join("");
    const headerInclude = "id,name,options,points,published" + questionsCSV;
...
export function sameType(questions: Question[]): boolean {
    ...
    const ifSame = questions.filter(
        (question: Question): boolean => question.type === firstType
    );
    if (ifSame.length === questions.length) {
        return true;
    }
    return false;
}
