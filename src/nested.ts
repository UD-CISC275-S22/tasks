...
export function getPublishedQuestions(questions: Question[]): Question[] {
    return questions.filter((questions) => questions.published === true);
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce(
        (accumulator: number, question: Question) =>
            question.published ? accumulator + question.points : accumulator,
        0
    );
}
...
export function toCSV(questions: Question[]): string {
    ...
    const string2 = questions.reduce(
        (current: string, option: Question) =>
            current +
            option.id +
            "," +
            option.name +
...
export function sameType(questions: Question[]): boolean {
    ...
    return questions.every(
        (question: Question): boolean => question.type === allType
    );
}
...
export function addNewQuestion(
    ...
    const question2 = makeBlankQuestion(id, name, type);
...
export function changeQuestionTypeById(
    ...
    return questions.map((question) => {
        // I dont know if I'm allowed to use if statements but when I used ? it kept giving me prettier errors and then indenting errors
        // and it was an infinite loop of errors so I just switched it to if statements which map through it and return the question
        // based on the instructions instruct
