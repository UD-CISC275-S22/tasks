...
export function sameType(questions: Question[]): boolean {
    const isShortAns = questions.every(
        (question: Question): boolean =>
            question.type === "short_answer_question"
    );
    const isMultChoice = questions.every(
        (question: Question): boolean =>
            question.type === "multiple_choice_question"
    );
    return isShortAns || isMultChoice;
}
...
export function addNewQuestion(
    ...
    const sameQuestionsWithBlank = questions.map(
        (question: Question): Question => ({
            ...question,
            options: [...question.options]
        })
    );
    sameQuestionsWithBlank.push(makeBlankQuestion(id, name, type));
    return sameQuestionsWithBlank;
}
...
export function duplicateQuestionInArray(
    ...
    const dup = duplicateQuestion(newId, questions[foundTarget]);
    ...
    duplicatedQuestions.splice(foundTarget + 1, 0, dup);
