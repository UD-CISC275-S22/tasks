...
export function findQuestion(
    ...
    const idMatch = questions.filter(
        (question: Question): boolean => question.id === id
    );
    return idMatch.length > 0 ? idMatch[0] : null;
}
...
export function sumPublishedPoints(questions: Question[]): number {
    ...
    const publishedPoints = publishedQuestions.reduce(
        (currentSum: number, question: Question) =>
            currentSum + question.points,
        0
    );
...
export function sameType(questions: Question[]): boolean {
    ...
    const typeCheck = questions.every(
        (question: Question) => question.type === base.type
    );
    return typeCheck;
}
...
export function addNewQuestion(
    ...
    const questionsWithBlank = [
        ...questions,
        makeBlankQuestion(id, name, type)
    ];
    return questionsWithBlank;
}
...
export function renameQuestionById(
    ...
    const questionsWithRenamed = questions.map(
        (question: Question): Question => ({
            ...question,
            name: question.id === targetId ? newName : question.name
        })
    );
    return questionsWithRenamed;
}
...
export function duplicateQuestionInArray(
    ...
    const idMatch = questions.findIndex(
        (question: Question): boolean => question.id === targetId
    );
    newArray.splice(
        idMatch + 1,
        0,
        duplicateQuestion(newId, questions[idMatch])
    );
    return newArray;
}
