...
export function isCorrect(question: Question, answer: string): boolean {
    const newEx: string = question.expected.trim().toLowerCase();
    const newAns: string = answer.trim().toLowerCase();
    if (newAns === newEx) {
        return true;
    }
    return false;
...
export function toShortForm(question: Question): string {
    const newName: string =
        question.name.length > 10
            ...
            : question.name;
    return question.id.toString() + ": " + newName;
}
...
export function addOption(question: Question, newOption: string): Question {
    const newOpp = [...question.options, newOption];
    const newQ = { ...question, options: newOpp };
    return newQ;
}
