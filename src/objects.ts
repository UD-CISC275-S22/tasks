...
export function makeBlankQuestion(
    ...
    //used chatGPT for this code and to understand instructions for this part
    return {
        id: id,
        name: name,
        type: type,
...
export function toMarkdown(question: Question): string {
    let mcString: string = "# " + question.name + "\n" + question.body;
    for (let i = 0; i < question.options.length; i++) {
        mcString += "\n- " + question.options[i];
    }
    const othString: string = "# " + question.name + "\n" + question.body;
    if (question.type === "multiple_choice_question") {
        return mcString;
    }
    return othString;
