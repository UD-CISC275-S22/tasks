...
export function isCorrect(question: Question, answer: string): boolean {
    if (
        ...
        question.expected.toString().trim().toLowerCase()
    ) {
        return true;
    }
    return false;
...
export function isValid(question: Question, answer: string): boolean {
    ...
    if (question.type == "multiple_choice_question") {
        //if in loop of options
        const validMultChoice = question.options.filter(
            (option: string): boolean => answer == option
        );
        if (validMultChoice.length > 0) {
            return true;
        }
    }
    return false;
...
export function toMarkdown(question: Question): string {
    ...
    const bulletPoints = question.options.join("\n- ");
    return "# " + question.name + "\n" + question.body + "\n- " + bulletPoints;
}
...
export function publishQuestion(question: Question): Question {
    if (question.published == true) {
        const updatedStatus = { ...question, published: false };
        return updatedStatus;
    }
    if (question.published == false) {
        const updatedStatus = { ...question, published: true };
        return updatedStatus;
    }
    return question;
}
