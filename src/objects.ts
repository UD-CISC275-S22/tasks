...
export function isCorrect(question: Question, answer: string): boolean {
    return question.expected.replace(/\s/g, "").toLowerCase() ===
...
export function isValid(question: Question, answer: string): boolean {
    ...
    else return question.options.includes(answer);
}
...
export function toShortForm(question: Question): string {
    return (
        question.id +
        ": " +
        question.name.slice(0, Math.min(question.name.length, 10))
    );
}
...
export function toMarkdown(question: Question): string {
    const head: string = "# " + question.name + "\n" + question.body + "\n";
    return question.options
        .reduce((curr: string, x) => (curr += "- " + x + "\n"), head)
        .trim();
}
