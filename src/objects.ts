...
export function isCorrect(question: Question, answer: string): boolean {
    //console.log(answer + " " + question.expected);
    return (
...
export function isValid(question: Question, answer: string): boolean {
    if (question.type === "short_answer_question") {
        ...
        const checkAns = question.options.find(
            (str: string): boolean => str === answer
        );
        //console.log(checkAns + " " + answer);
        return checkAns === undefined ? false : true;
    } else {
        return false;
    }
}
...
export function toShortForm(question: Question): string {
    ...
    const result = queId + ": " + question.name.slice(0, 10);
    return result;
}
...
export function toMarkdown(question: Question): string {
    ...
    const result2 = line2.join("\n");
    if (question.type === "multiple_choice_question") {
        const showOpt = question.options.map(
            (str: string): string => "- " + str.toString()
        );
        const result = showOpt.join("\n");
        //console.log(result1 + "\n" + result2 + "\n" + result);
        return result1 + "\n" + result2 + "\n" + result;
    } else {
        return result1 + "\n" + result2;
...
export function renameQuestion(question: Question, newName: string): Question {
    //console.log(question.name + " " + newName); {...question.name,}
    const result = { ...question, name: newName };
    return result;
}
...
export function publishQuestion(question: Question): Question {
    ...
    if (question.published === false) {
        const changeToTrue = { ...question, published: true };
        return changeToTrue;
    } else {
        const changeToFalse = { ...question, published: false };
        return changeToFalse;
    }
}
