...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    const trimExp = question.expected.trim().toLowerCase();
    return trimmed === trimExp;
}
...
export function toShortForm(question: Question): string {
    let start = question.id.toString();
    start += ": ";
    start += question.name.substring(0, 10);
    return start;
}
...
export function toMarkdown(question: Question): string {
    let done = "# " + question.name + "\n";
    done += question.body + "\n";
    if (question.type === "multiple_choice_question") {
        for (const loop of question.options) {
            done += "- " + loop + "\n";
        }
    }
    return done.trimEnd();
...
export function addOption(question: Question, newOption: string): Question {
    const newQ = [...question.options, newOption];
    return { ...question, options: newQ };
}
