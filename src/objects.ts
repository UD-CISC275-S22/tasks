...
export function isCorrect(question: Question, answer: string): boolean {
    const standardQuestion = question.expected.toLowerCase().trim();
    const standardAnswer = answer.toLowerCase().trim();
    return standardQuestion === standardAnswer;
}
...
export function toShortForm(question: Question): string {
    const Qstring = question.id + ": " + question.name.slice(0, 10);
    return Qstring;
}
...
export function toMarkdown(question: Question): string {
    if (question.type === "multiple_choice_question") {
        const optionList = question.options.map(
            (option: string): string => "- " + option.toString() + "\n"
        );
        const options = optionList.join("");
        const optionsMinusEnd = options.slice(0, options.length - 1);
        return (
            "# " + question.name + "\n" + question.body + "\n" + optionsMinusEnd
        );
    }
