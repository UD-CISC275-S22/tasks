...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    const newAnswer = trimmed.toLowerCase();
    const newQ = question.expected.toLowerCase();
    const newQ2 = newQ.trim();
    if (newAnswer === newQ2) {
        return true;
    } else {
        return false;
...
export function toShortForm(question: Question): string {
    const newArr = question.name.substring(0, 10);
    return question.id + ": " + newArr;
}
...
export function toMarkdown(question: Question): string {
    let formattedString: string = "# " + question.name + "\n" + question.body;
