...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    return ans === question.expected.toLowerCase() ? true : false;
}
...
export function isValid(question: Question, answer: string): boolean {
    if (question.type === "short_answer_question") {
        ...
        const val = (ans: string): boolean => ans === answer;
        return question.options.some(val);
    } else {
        return false;
    }
}
...
export function toMarkdown(question: Question): string {
    const first_line = "# " + question.name + "\n";
    const second_line = question.body + "\n";
    const mc_options = question.options.reduce(
        (currentString: string, opt: string) =>
            currentString + "- " + opt + "\n",
        ""
    );
...
export function renameQuestion(question: Question, newName: string): Question {
    const new_q = { ...question, name: newName };
    return new_q;
}
...
export function publishQuestion(question: Question): Question {
    const new_q = {
        ...question,
        published: question.published === true ? false : true
    };
    return new_q;
}
