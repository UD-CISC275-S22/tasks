...
export function isCorrect(question: Question, answer: string): boolean {
    const finalAnswer = answer.trim().toLowerCase();
...
export function toShortForm(question: Question): string {
    const idn = question.id;
    const fullName = question.name;
    const shortName = fullName.slice(0, 10);
    return idn.toString() + ": " + shortName.toString();
}
...
export function toMarkdown(question: Question): string {
    ...
    if (question.type === "multiple_choice_question") {
        ...
        markdown +=
            "\n" +
            options.map((option: string): string => "- " + option).join("\n");
    }
    return markdown;
}
...
export function publishQuestion(question: Question): Question {
    const publishedQuestion = { ...question, published: !question.published };
