...
export function isCorrect(question: Question, answer: string): boolean {
    const trimmedQuestion = { ...question, expected: question.expected.trim() };
    const trimmedAnswer = answer.trim();
    return (
        trimmedQuestion.expected.toLowerCase() === trimmedAnswer.toLowerCase()
    );
}
...
export function isValid(question: Question, answer: string): boolean {
    if (question.type === "short_answer_question") {
        return true;
    } else if (question.options.indexOf(answer) !== -1) return true;
    else return false;
}
