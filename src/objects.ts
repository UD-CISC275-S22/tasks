...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    const lowerExpectedAns = question.expected.trim().toLowerCase();
    return lowerAns === lowerExpectedAns;
}
...
export function toShortForm(question: Question): string {
    const short = question.name.slice(0, 10);
...
export function renameQuestion(question: Question, newName: string): Question {
    return {
        ...question, // Copy all properties of the original question
        name: newName // Override the name property with newName
    };
}
...
export function addOption(question: Question, newOption: string): Question {
    return {
        ...question,
        options: [...question.options, newOption] // Create a new array with existing options and add the new option
    };
}
