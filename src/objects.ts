...
export function isCorrect(question: Question, answer: string): boolean {
    const editedAnswer = answer.toLowerCase().trim();
    const editedQuestion = question.expected.toLowerCase().trim();
    return editedAnswer == editedQuestion;
}
...
export function isValid(question: Question, answer: string): boolean {
    ...
    if (questionType == "multiple_choice_question") {
        const inOptions = question.options.filter(
            (str: string): boolean => answer == str
        );
...
export function toShortForm(question: Question): string {
    const firstTen = question.name.split(question.name[10], 10);
    return question.id + ": " + firstTen[0];
}
...
export function toMarkdown(question: Question): string {
    const string = "# " + question.name + "\n" + question.body;
    if (question.type == "multiple_choice_question") {
        let listOfOptions = question.options.join("\n- ");
        listOfOptions = "\n- " + listOfOptions;
        return string + listOfOptions;
    } else {
        return string;
    }
}
...
export function publishQuestion(question: Question): Question {
    if (question.published == true) {
        const invertedPublished = { ...question, published: false };
        return invertedPublished;
    } else {
        const invertedPublished = { ...question, published: true };
        return invertedPublished;
    }
}
