...
export function isValid(question: Question, answer: string): boolean {
    //first test for short answer, any answer is valid
    if (question.type === "short_answer_question") {
        return true;
    } else if (question.type === "multiple_choice_question") {
        //multiple choice must be one of the listed options
        const options = question.options.includes(answer);
        return options;
    } else {
        return false;
    }
...
export function addOption(question: Question, newOption: string): Question {
    const addOption = [...question.options];
    addOption.push(newOption);
    return {
        ...question,
        options: addOption
    };
